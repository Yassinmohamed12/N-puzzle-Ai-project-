...
#     # Calculate the linear conflict cost for a state
#     conflicts = 0
#     size = len(state)
...
#                 continue
#             xx,yy=element_position(state[i][j],goal)
#             goal_positionx = divmod(xx, size)
#             goal_positiony = divmod(yy, size)
#             if goal_positionx == i:  # Check for conflicts in the same row
#                 for k in range(j + 1, size):
#                     if state[i][k] != 0 and goal[state[i][k] - 1] / size == i and state[i][j] > state[i][k]:
#                         conflicts += 1
#             elif goal_positiony == j:  # Check for conflicts in the same column
#                 for k in range(i + 1, size):
#                     if state[k][j] != 0 and goal[state[k][j] - 1] % size == j and state[i][j] > state[k][j]:
#                         conflicts += 1
#     return 2 * conflicts + manhattan_distance(state, goal)  # Each linear conflict involves two tiles
